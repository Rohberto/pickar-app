...
export default function UserChatScreen() {
  ...
  // Clear unread badge when this screen mounts
  ...
  const connectSocket = () => {
    ...
    socket.on('connect', () => {
      ...
      socket.emit('mark_read', { deliveryId, readerType: 'user' });
    });

    // Our message confirmed — replace temp with real DB entry
  ...
  const handleSend = () => {
    ...
    socketRef.current?.emit('send_message', {
      deliveryId, senderId: user?.id, senderType: 'user', message: text,
    });
  };

  const handleCall = () => {
    if (!driverPhone) {
      Alert.alert('Unavailable', "Driver's number is not available.");
      return;
    }
  ...
  // Safe back — won't crash if there's no screen to go back to
  const handleBack = () => {
    if (router.canGoBack()) {
      router.back();
    } else {
        ...
        <View style={[styles.row, isMe && styles.rowMe]}>
          {!isMe && (
            <View style={styles.avatarSmall}>
              {driverPhoto
                ? <Image source={{ uri: driverPhoto }} style={styles.avatarSmallImg} />
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerInfo}>
          <View style={styles.headerAvatar}>
            {driverPhoto
              ? <Image source={{ uri: driverPhoto }} style={styles.headerAvatarImg} />
          ...
          <FlatList
            ...
            ListEmptyComponent={
              <View style={styles.empty}>
                ...
                <Text style={styles.emptySub}>Say hi to your driver</Text>
              </View>
            }
          />
        )}
